 ...
 * Author: Christian Schafmeister (1991)
 *
 * Sort the three non-central improper dihedral atom types into
 * alphabetical order to speed up parameter searches.
 * Atom 3 (index 2) is always the central atom and is never moved.
 *
 * saAtoms[4] and sOrder[4] use the same 0-based indices 0,1,2,3
 * corresponding to sAtom1,2,3,4. sOrder arrives as "0123" and
 * tracks the resulting permutation.
 *
 * WILD_CARD_TYPE is defined as '!' (ASCII 33) which sorts before
...
#define SWAP_ATOMS(i, j)                                \
     ...
     * after [0,1] + [1,3], largest is guaranteed at [3];
     * final [0,1] finishes the sort. */
...
//---------------------------------------------------------------------------------------------
// iParmSetAddBond: Add a bond parameter to the PARMSET.  Return the index of the newly added
//                  band parameter.
//
// Arguments:
//   
	...
	(strcmp(PVAI(vaPNBEdits, NBEDITt, i)->sType1, tJ) == 0 &&
	 strcmp(PVAI(vaPNBEdits, NBEDITt, i)->sType2, tI) == 0)) {
      ...
      MathOpConvertNonBondToAC(PVAI(vaPNBEdits, NBEDITt, i)->dEI,
			       ...
			       PVAI(vaPNBEdits, NBEDITt, i)->dRJ, dA, dB);
      break;
    }
  }
}
...
//---------------------------------------------------------------------------------------------
// ParmSetBond: Return a bond parameter in the ParmSet
//
// Arguments:
...
//   dPKb:       bond stiffness constant
//   dPR0:       bond equilibrium length
//   dPKpull:    stiffness constant for pulling overtone
//   dPRpull0:   length at which the pulling overtone kicks in
...
//---------------------------------------------------------------------------------------------
// ParmSetUpdateBond: return a bond parameter in the ParmSet, with changes if any of the
//                    inputs are not NULL.  Atom types are pre-ordered if changed.
//
// Arguments:
//   psLib:      the library of available parameters, including bonds
//   i:          the index of the bond to pull out
                       ...
                       double *dPKpress, double *dPRpress0, char *sDescription)
{
  ...
  bpPBond = PVAI( psLib->vaBonds, BONDPARMt, i );
  if (sType1 != (char*)NULL) {
    strcpy(bpPBond->sType1, sType1);
  }
  if (sType2 != (char*)NULL) {
    strcpy(bpPBond->sType2, sType2);
  }
  if (dPKb != (double*)NULL) {
    bpPBond->dKb = *dPKb;
  }
  ...
  if (dPKpull != (double*)NULL) {
    bpPBond->dKpull = *dPKpull;
  }
  if (dPRpull0 != (double*)NULL) {
    bpPBond->dRpull0 = *dPRpull0;
  }
  if (dPKpress != (double*)NULL) {
    bpPBond->dKpress = *dPKpress;
  }
  if (dPRpress0 != (double*)NULL) {
