...
module xray_non_bulk_impl_gpu_module
  ...
  interface
    subroutine pmemd_xray_non_bulk_init_gpu(n_hkl, hkl, f_non_bulk, mSS4, n_atom, &
        & na, nb, nc, sgn, b_factor, occupancy, &
  ...
  interface
    subroutine pmemd_xray_non_bulk_calc_f_non_bulk_gpu(n_atom, na, nb, nc, sgn, frac_xyz) bind(C)
      use iso_c_binding
      implicit none
      integer(c_int), value :: n_atom
      integer(c_int), value :: na, nb, nc, sgn
...
contains
  ...
  subroutine calc_f_non_bulk(frac)
    ...
    call pmemd_xray_non_bulk_calc_f_non_bulk_gpu(size(frac, 2), na, nb, nc, sgn, frac)
  ...
  subroutine gpu_init()
    ...
    call pmemd_xray_non_bulk_init_gpu( &
        & size(hkl, 2), hkl, f_non_bulk, mSS4, size(b_factor), na,nb,nc,sgn, &
        & b_factor, &
        & occupancy, &
