...
subroutine  debug_mem(natom,istartr,iendr)
...
#  include "ew_mpole.h"
   ...
   _REAL_ apavir,apmvir,exavir,exmvir
   ...
   ene%volume   = volume
   if ( master )then
      call prntmd(nstep,t,ene,onefac,iout7,.false.)
   end if
   if ( dumpfrc == 1 )then
      call force_dump(natom,xx,ix,ih,ipairs,x,f, &
            ener,xx(lscg),xx(lscn1),xx(lscn2),xx(l15), &
            cn1,cn2,xx(linddip),xx(lsind),qsetup)
   end if
   if ( rmsfrc == 1 )then
      call rms_check(natom,xx,ix,ih,ipairs,x,f, &
            ener,xx(lsrms),xx(lf2),xx(lf3),xx(lf4),xx(lf5),xx(lf6), &
            xx(lscg),xx(lscn1),xx(lscn2),xx(l15),cn1,cn2, &
   ...
   do j = 1,natomn
      ...
      call get_numfrc(xx,ix,ih,ipairs,x,f, &
            ener%vir,ener,atomnum,del,apfrc,qsetup)
...
subroutine crd_rst(crd,atomn,j,save)
   implicit none
   _REAL_ crd(3,*),save
   integer atomn,j
...
subroutine rms_check(natom,xx,ix,ih,ipairs,x,f, &
      ener,srms,f2,f3,f4,f5,f6,scg,scn1,scn2,cg,cn1,cn2, &
      inddip,sind,qsetup)
...
#  include "flocntrl.h"
   ...
   _REAL_ srms(*),f2(3,*),f3(3,*),f4(3,*), &
         f5(3,*),f6(3,*)
   _REAL_ scg(*),scn1(*),scn2(*),cg(*),cn1(*),cn2(*)
   type(state_rec) :: ener, ene
   integer ipairs(*),ix(*)
   character(len=4) ih(*)
   integer j,natom,jmax,matom
...
#  include "../include/md.h"
   ...
   call force(xx,ix,ih,ipairs,x,f,ene,ene%vir, &
         xx(l96), xx(l97), xx(l98), xx(l99), qsetup, do_list_update,0 )
   enep = ene%pot%tot
   volp = volume
   ! put cell back
...
#  include "../include/md.h"
   ...
   _REAL_ xx(*),x(*),f(*), &
         del,duda(3,3),vten(3,3),apduda(3,3)
   integer ipairs(*),ix(*),type
   character(len=4) ih(*)
...
subroutine do_percentiles(srms,max,min,natom)
   ...
   integer imed,isev5,ininety,ininety5,ininety9,j,k
