...
! CHA-GB input radii are must. Tested with ALPB version of CHA-GB (AM and SI :
! 12/17/2013)
subroutine cha_rad(isymbl,natom, gb_Rs ,radi,rin)
   implicit none
...
!print*, "insidecha"
 do iatm = 1, natom
!    print*, "isymbl",isymbl(iatm)
    if (( isymbl(iatm)(1:1) == 'H') .OR. (isymbl(iatm)(1:1) == 'h')) then 
      radi(iatm) = 0.52d0
    else if (( isymbl(iatm)(1:2) == 'Cl') .OR. (isymbl(iatm)(1:2) == 'cl')) then 
      radi(iatm) = 1.63d0
      write(6,'(a,a)') 'Warning: CHA-GB radius is optimized as part of a molecule',isymbl(iatm)(1:2)
    else if (( isymbl(iatm)(1:2) == 'Cs' ).OR.( isymbl(iatm)(1:2) == 'cs')) then 
      radi(iatm) = 1.70d0
      write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
    else if((( isymbl(iatm)(1:1) == 'C').OR. (isymbl(iatm)(1:1) == 'c')) &
      &.OR.((isymbl(iatm)(1:2) == '2C').OR.(isymbl(iatm)(1:2) == '3C'))) then
      radi(iatm) = 1.60d0
    else if ( isymbl(iatm)(1:2) == 'Na' ) then 
      radi(iatm) = 1.02d0
      write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
       ...
       radi(iatm) = 1.22d0
       write(6,'(a,a)') 'Warning: CHA-GB radius is optimized as part of a molecule',isymbl(iatm)(1:1)
    else if ((( isymbl(iatm)(1:1) == 'I') .OR. (isymbl(iatm)(1:1) == 'i')).AND. &
       & (( isymbl(iatm)(1:2) .ne. 'IP' ).AND.( isymbl(iatm)(1:2) .ne. 'IM' ))) then 
       radi(iatm) = 2.14d0
       write(6,'(a,a)') 'Warning: CHA-GB radius is optimized as part of a molecule',isymbl(iatm)(1:2)
    else if (( isymbl(iatm)(1:2) == 'Br') .OR. (isymbl(iatm)(1:2) == 'br')) then 
       radi(iatm) = 1.84d0
       write(6,'(a,a)') 'Warning: CHA-GB radius is optimized as part of a molecule',isymbl(iatm)(1:2)
     else if (( isymbl(iatm)(1:2) == 'Li' ).OR.( isymbl(iatm)(1:2) == 'li' )) then 
        radi(iatm) = 0.74d0
        write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
     else if (( isymbl(iatm)(1:1) == 'K' ).OR.( isymbl(iatm)(1:1) == 'k' )) then 
        radi(iatm) = 1.38d0
        write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
     else if (( isymbl(iatm)(1:2) == 'Rb' ).OR.( isymbl(iatm)(1:2) == 'rb' )) then 
        radi(iatm) = 1.49d0
        write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
     else if ((( isymbl(iatm)(1:2) == 'MG').OR.( isymbl(iatm)(1:2) == 'Mg')).OR. &
     & ( isymbl(iatm)(1:2) == 'mg' )) then 
        radi(iatm) = 0.72d0
        write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
     else if (( isymbl(iatm)(1:2) == 'Zn' ).OR.( isymbl(iatm)(1:2) == 'zn')) then 
      radi(iatm) = 0.75d0  
        write(6,'(a,a)') 'Warning: CHA-GB ion radius from Marcus et.al.: &
                &OK for isolated and in solvent, but not as part of complex',isymbl(iatm)(1:2)
     else
     radi(iatm) = rin(iatm)
     write(6, '(a,i6,a)') 'GB Bomb in cha_rad: Atom type not recognizable, radius assigned from topology',iatm, isymbl(iatm)
     write(6,'(a)') 'Warning: CHA-GB is optimized for C, N, O, H, S, P, Cl, F, Br, I atom types only'
     endif 
end do
   ! safe-guarding, very important!
   do iatm = 1, natom
      if ( radi(iatm) == ZERO ) then
         write(6, '(a,i6,a)') 'GB Bomb in cha_rad(): No radius assigned for atom', iatm, isymbl(iatm)
         call mexit(6, 1)
      endif
      radi(iatm) = radi(iatm) + gb_Rs
!     print*, "gb_cha for", iatm," is", radi(iatm)-gb_Rs
   enddo


end subroutine cha_rad 


!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
!+ PB cavity radi setup based on electrostatic solvation free energies in TIP3P
subroutine cha_aaradi( natom, nbonh, ibh, jbh, radi, acrg, ucrgh, ucrga, resid, igraph, isymbl, rin )
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   !
...
#  include "pb_constants.h"
   ...
   do iatm = 1, natom
       ...
       if     ( isymbl(iatm)(1:1) == 'H' ) radi(iatm) = 1.04d0
   ...
   do iatm = 1, natom
      ...
      elseif     ( isymbl(iatm)(1:2) == 'CT' ) then
               radi(iatm) = 2.12d0
   ...
   do iatm = 1, natom
      ...
      if ( radi(iatm) == ZERO ) then
         write(6,'(a,i6,2(2x,a4))') 'PB Bomb in pb_aaradi(): No radius assigned for atom', iatm, igraph(iatm), isymbl(iatm)
         call mexit(6, 1)
      endif

   enddo

end subroutine cha_aaradi
...
subroutine sa_arc( verbose,pbprint,natom,atmfirst,atmlast )
   ...
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5) /= 0 ) then
      write(6, '(a,12i6)') 'SA Bomb in sa_arc()1: Allocates aborted', alloc_err(1:5)
      call mexit(6, 1)
   end if
   ...
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4) /= 0 ) then
      write(6,'(a,4i6)') 'SA Bomb in sa_arc()2: Allocates aborted', alloc_err(1:4)
      call mexit(6, 1)
   end if
   ...
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5) /= 0 ) then
      write(6, '(a,5i6)') 'SA Bomb in sa_arc()3: Allocates aborted', alloc_err(1:5)
      call mexit(6, 1)
   end if
...
!        write(6,*) 'trimer',triatm(1:3,isph)
   ...
   deallocate(knocktri)
   if ( alloc_err(1)+alloc_err(2)+alloc_err(3)+alloc_err(4)+alloc_err(5) /= 0 ) then
      write(6, '(a,5i6)') 'SA Bomb in sa_arc(): Deallocation aborted', alloc_err(1:5)
      call mexit(6, 1)
   end if
   call pbtimer_stop(PBTIME_PBSAARC_SETUP)
