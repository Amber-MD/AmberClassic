...
int make_RST_restraint( int tnum, int rnum, char *rname, char *angname,
                        float lb, float ub );
int read_lib();
int residue_offset( char *a, char *b );
...
#define	DEFAULT_LIB "dat/tordef.lib"
...
float *get_bounds( float *bvec, int j, float P_lower, float P_upper)
{
	float	nu;
