...
void rotate0(ATOM obj1, ATOM obj2, ATOM obj3, ATOM * obj4, double bond, double angle,
             double twist)
{
...
double angle0(ATOM obj1, ATOM obj2, ATOM obj3, double *ww1, double *ww2, double *ww3)
{
    double w1, w2, w3, w4;
    double xa, ya, za, xc, yc, zc;
    double xa3, ya3, za3, xc3, zc3;
...
double rotate00(ATOM obj1, ATOM obj2, ATOM obj3, ATOM * obj4, double twist,
                int AngleChange)
{
...
void intercoord(int atomnum, ATOM atom[], char *torstr)
{
    ...
    for (i = 3; i < atomnum; i++) {
        ...
        for (j = 0; j < 6; j++)
            if (select[atom[i].con[j]] == 1) {
                ...
                if (tmpint1 == -1)
                    continue;
                for (k = 0; k < 6; k++)
                    if (select[atom[tmpint1].con[k]] == 1 && atom[tmpint1].con[k] != i) {
        ...
        atom[i].angle =
            anglecal(atom[i], atom[atom[i].bondatom], atom[atom[i].angleatom]);
        atom[i].twist =
...
*/
        rotate0(atom[atom[i].twistatom], atom[atom[i].angleatom], atom[atom[i].bondatom],
                &atom[i], atom[i].bond, atom[i].angle, atom[i].twist);
    }
}



int connect(char *connect_file, int atomnum, ATOM * atom, int *bondnum, BOND * bond,
            int maxbond)
{
    int i, j;
    int tmpint;
    ...
    for (i = 0; i < atomnum - 1; i++)
        for (j = i + 1; j < atomnum; j++)
            if (((atom[i].resno - atom[j].resno) == 1 && atom[j].ter != 1)
                ...
                ref = connectradius[atom[i].atomicnum] + connectradius[atom[j].atomicnum];
                if (ref <= 1.5)
                    offset = ref * 0.15;
                if (ref > 1.5 && ref <= 1.90)
                    offset = ref * 0.11;
...
void omegarotate2(ATOM atom[], int atomnum, ATOM refatom, double angle, double w1,
                  double w2)
{
    double x, y, z;
    double x2, y2, z2;
