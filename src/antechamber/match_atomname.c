...
void wprepi(char *input_filename, char *output_filename, int atomnum, ATOM atom[])
{
    ...
    if ((fpin = fopen(input_filename, "r")) == NULL) {
        fprintf(stdout, "Warning: Cannot open file (%s) to read.\n",
                input_filename);
        return;
    }
    ...
    for (;;) {
        ...
        if (flag == 1) {
            strcpy(atomname, "");
            atomname[0] = '\0';
            sscanf(line, "%d%s%s%s%d%d%d%lf%lf%lf%lf", &id, tmpchar1, tmpchar2, tmpchar3,
                   &tmpint1, &tmpint2, &tmpint3, &x, &y, &z, &c);
            if (ihydrogen == 1 || (ihydrogen == 0 && tmpchar1[0] != 'H')) {
                ...
                if (suc == 0) {
                    eprintf("Cannot find matched atom names for atom name (%s).\n",
                            tmpchar1);
                }
            } else
                strcpy(atomname, tmpchar1);
            fprintf(fpout, "%4d  %-6s%-6s%-4s%-4d%-4d%-4d%8.3lf%10.3lf%10.3lf%10.6f\n",
                    id, atomname, tmpchar2, tmpchar3, tmpint1, tmpint2, tmpint3, x, y, z,
                    c);
        }
        if (flag == 2) {
            strcpy(atomname1, "");
...
void wprepc(char *input_filename, char *output_filename, int atomnum, ATOM atom[])
{
    ...
    for (;;) {
        ...
        if (flag == 1) {
            strcpy(atomname, "");
            atomname[0] = '\0';
            sscanf(line, "%d%s%s%s%lf%lf%lf%lf", &id, tmpchar1, tmpchar2, tmpchar3, &x,
                   &y, &z, &c);
            if (ihydrogen == 1 || (ihydrogen == 0 && tmpchar1[0] != 'H')) {
                suc = 0;
                for (i = 0; i < atomnum; i++) {
                ...
                strcpy(atomname, tmpchar1);
            fprintf(fpout, "%4d  %-6s%-6s%-4s%15.6lf%12.6lf%12.6lf%12.6f", id, atomname,
                    tmpchar2, tmpchar3, x, y, z, c);
        }
        if (flag == 2) {
            strcpy(atomname1, "");
            atomname1[0] = '\0';
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(iformat, "pdb") != 0 && strcmp(iformat, "ac") != 0
        && strcmp(iformat, "prepi") != 0 && strcmp(iformat, "prepc") != 0
        && strcmp(iformat, "mol2") != 0) {
    ...
    ref_atom = (ATOM *) emalloc(sizeof(ATOM) * ref_cinfo.maxatom);

    bond = (BOND *) emalloc(sizeof(BOND) * cinfo.maxbond);
    ref_bond = (BOND *) emalloc(sizeof(BOND) * ref_cinfo.maxbond);
...
/*for ref atom */
    if (strcmp(rformat, "ac") == 0) {
        overflow_flag =
            rac(rfilename, &ref_atomnum, ref_atom, &ref_bondnum, ref_bond, &ref_cinfo,
                &ref_minfo);
    }
    if (strcmp(rformat, "pdb") == 0) {
        overflow_flag = rpdb(rfilename, &ref_atomnum, ref_atom, ref_cinfo, ref_minfo, 0);
    }
    if (strcmp(rformat, "mol2") == 0) {
        overflow_flag =
            rmol2(rfilename, &ref_atomnum, ref_atom, &ref_bondnum, ref_bond, &ref_cinfo,
                  &ref_minfo, 0);
    }
    if (strcmp(rformat, "prepi") == 0) {
        overflow_flag =
            rprepi(rfilename, &ref_atomnum, ref_atom, &ref_bondnum, ref_bond, &ref_cinfo,
                   &ref_minfo);
    }
    if (strcmp(rformat, "prepc") == 0) {
        overflow_flag = rprepc(rfilename, &ref_atomnum, ref_atom, &ref_cinfo, &ref_minfo);
    }
    ...
    if (overflow_flag) {
        ...
        if (strcmp(rformat, "mol2") == 0) {
            overflow_flag =
                rmol2(rfilename, &ref_atomnum, ref_atom, &ref_bondnum, ref_bond,
                      &ref_cinfo, &ref_minfo, 0);
        }
        if (strcmp(rformat, "prepi") == 0) {
            overflow_flag =
                rprepi(rfilename, &ref_atomnum, ref_atom, &ref_bondnum, ref_bond,
                       &ref_cinfo, &ref_minfo);
        }
        if (strcmp(rformat, "prepc") == 0) {
            overflow_flag =
    ...
    input_atom = (ATOM *) emalloc(sizeof(ATOM) * cinfo.maxatom);
    input_bond = (BOND *) emalloc(sizeof(BOND) * cinfo.maxbond);

    if (cinfo.maxatom > ref_cinfo.maxatom)
        hcount = (int *) emalloc(sizeof(int) * cinfo.maxatom);
    else
        hcount = (int *) emalloc(sizeof(int) * ref_cinfo.maxatom);
...
/*      for connect.tpl and radius parameter files */
    ...
    build_dat_path(ref_minfo.connect_file, "CONNECT.TPL", sizeof ref_minfo.connect_file,
                   0);
    ...
    if (strcmp(iformat, "pdb") == 0 || strcmp(iformat, "prepc") == 0
        || strcmp(iformat, "prepi") == 0) {
        connect(minfo.connect_file, atomnum, atom, &bondnum, bond, cinfo.maxbond);
    }
    if (strcmp(rformat, "pdb") == 0 || strcmp(rformat, "prepc") == 0
        || strcmp(rformat, "prepi") == 0) {
        connect(ref_minfo.connect_file, ref_atomnum, ref_atom, &ref_bondnum, ref_bond,
                ref_cinfo.maxbond);
    }
...
/* 	continue to allocate memory */
    input_equ_atom_id = (int *) emalloc(sizeof(int) * input_atomnum);
    input_atom_id = (int *) emalloc(sizeof(int) * input_atomnum);
    ref_equ_atom_id = (int *) emalloc(sizeof(int) * ref_atomnum);
    ref_atom_id = (int *) emalloc(sizeof(int) * ref_atomnum);
    for (i = 0; i < input_atomnum; i++) {
    ...
    wac("MATCH_ATOMNAME_INPUT.AC", input_atomnum, input_atom, input_bondnum, input_bond,
        cinfo, minfo);
    wac("MATCH_ATOMNAME_REF.AC", ref_atomnum, ref_atom, ref_bondnum, ref_bond, ref_cinfo,
        ref_minfo);
/*	begin the real work */
    if (input_atomnum != ref_atomnum) {
        printf
            ("Error: The numbers of atoms are different for the input and the reference\n"
    ...
    match_atompair(input_atomnum, ref_atomnum, input_atom, ref_atom, input_equ_atom_id,
                   ref_equ_atom_id, input_atom_id, ref_atom_id, input_bondnum,
                   ref_bondnum, input_bond, ref_bond, max_path_length, igeom);
    if (debug == 1) {
        for (i = 0; i < input_atomnum; i++) {
            if (input_atom_id[i] >= 0)
                printf("Input %5d %5d %5s %5s\n", i + 1, input_atom_id[i] + 1,
                       input_atom[i].name, ref_atom[input_atom_id[i]].name);
            else
    ...
    for (i = 0; i < atomnum; i++) {
        if (ihydrogen == 0) {
            ...
            if (input_atom_id[numatom] >= 0)
                ...
                fprintf(stdout, "Warning: Cannot find the substituted atom name (%s)\n"
                                "         in the reference for ATOM (%5d).\n",
                        atom[i].name, i + 1);
            numatom++;
        } else {
            if (input_atom_id[i] >= 0)
